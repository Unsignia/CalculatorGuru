...
import { quadraticequation } from "./../../utils/geoformula.js";

export default function QuadraticEquation() {
  const [missing, setMissing] = useState("x");
  ...
  function handleSubmit(e) {
    e.preventDefault();
    var c = quadraticequation(missing, x, a, b, c);
  }
  ...
  return (
    <>
      <Title title="Quadratic Equation" />
      <Header />

      <form className="w-3/4 mx-auto" onSubmit={handleSubmit}>
        <h1 className="py-8 text-center text-lg">Quadratic Equation</h1>
        <div className="flex">
          <div className="w-3/4">
            <label
              htmlFor="names"
            ...
            <select
              ...
              value={missing}
            >
              <option value="x">Solve for x</option>
            </select>
          </div>
        </div>
        ...
        <div className="my-4 bg-gray-50 border border-gray-300 text-gray-800 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
          The answer is:
          {" " + quadraticequation(missing, x, a, b, c)}
        </div>
      </form>
    </>
  );
